<template>
	...
	<el-row :gutter="10" class="mb8">
		<el-col :span="24">
			<el-button :type="themeVariables.addBtnElType || 'primary'" plain icon="i-ep-plus" size="large" @click="openAddDialog">添加部门</el-button>
		</el-col>
	</el-row>
	<el-table
		:data="tableData"
	...
	>
		...
		<el-table-column prop="orderNum" label="排序" sortable width="100px" align="center" />
		<el-table-column prop="isStop" label="是否停用" sortable width="120px" align="center">
		...
		<el-table-column prop="createTimeStr" label="创建时间" sortable width="180px" />
		<el-table-column fixed="right" label="操作" width="300px" align="center">
			<template #default="scope">
				<el-button
					...
					size="small"
					icon="i-ep-plus"
					@click="openAddSubDeptDialog(scope.$index, scope.row)"
					>添加子级部门</el-button
				>
				<el-button
					text
	...
	<el-dialog
		:title="addOrModifyTag == 1 ? '添加部门' : '修改部门'"
		v-model="dialogFormAddOrModifyDept"
		width="500px"
	>
		<el-form
			:model="addOrModifyDeptForm"
			:rules="addOrModifyDeptFormRules"
			ref="addOrModifyDeptFormRef"
			label-width="80px"
		>
...
import {
	...
	modifyDept,
	deleteDept,
	getDoesNotContainSelfDeptTreeSelect,
...
const handleListTree = async () => {
	...
	const result = await listTreeDept(tableParams.value);
	try {
		if (result.code == 200) {
...
/*********添加部门*********/
//弹出框是否显示
const dialogFormAddOrModifyDept = ref(false);
//添加部门表单初始值
...
const openAddDialog = async () => {
	...
	//获取上级部门列表
	try {
		const result = await getDoesNotContainSelfDeptTreeSelect();
		if (result.code == 200) {
			parentDeptList.value = result.data;
		} else {
			ElMessage.error(result.msg);
		}
	} catch {
...
//打开添加子级弹框
const openAddSubDeptDialog = async (index: number, row: any) => {
	//添加编辑部门初始值
	addOrModifyDeptForm.value = { ...initialAddOrModifyDeptForm };
	//清除验证状态
	addOrModifyDeptFormRef.value?.clearValidate();
...
const openModifyDialog = async (index: number, row: any) => {
	...
	//获取上级部门列表
	try {
		const result = await getDoesNotContainSelfDeptTreeSelect(row.deptId);
		if (result.code == 200) {
			parentDeptList.value = result.data;
	...
	await formEl.validate(async (valid) => {
		if (valid) {
			...
			if (addOrModifyTag.value == 1) {
				const result = await addDept(addOrModifyDeptForm.value);
				if (result.code == 200) {
					ElMessage({
						message: "添加成功",
						type: "success",
				...
				const result = await modifyDept(addOrModifyDeptForm.value);
				if (result.code == 200) {
					ElMessage({
						message: "修改成功",
						type: "success",
...
const handleDelete = async (index: number, row: any) => {
	//调用删除方法
	try {
		const result = await deleteDept(row.deptId);
		if (result.code == 200) {
			ElMessage({
				message: "删除成功",
				type: "success",
