<template>
	...
	<el-table border stripe row-key="infoId" :data="tableData" class="custom-table" @sort-change="handleTableSort" v-loading="isLoading">
		<el-table-column
			prop="userName"
			label="用户账号"
			width="150"
		...
		<el-table-column
			...
			show-overflow-tooltip
		></el-table-column>
		<el-table-column prop="isSuccess" label="是否登陆成功" width="140" sortable align="center">
			<template #default="item">
				<el-tag v-if="item.row.isSuccess == 0" :type="themeVariables.tableElTagType || 'info'">否</el-tag>
				<el-tag v-if="item.row.isSuccess == 1" :type="themeVariables.tableElTagType || 'info'">是</el-tag>
			</template>
		</el-table-column>
		<el-table-column
			prop="msg"
...
import { listPageLogininfor, deleteLogininfor } from "@/api/system/logininfor/index";
...
const handleListPage = async () => {
	...
	const result = await listPageLogininfor(tableParams.value);
	try {
		if (result.code == 200) {
...
const handleDelete = async (index: number, row: any) => {
	//调用删除方法
	try {
		const result = await deleteLogininfor(row.infoId);
		if (result.code == 200) {
			ElMessage({
				message: "删除成功",
				type: "success",
