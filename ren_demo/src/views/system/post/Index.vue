<template>
	...
	<el-row :gutter="10" class="mb8">
		<el-col :span="24">
			<el-button :type="themeVariables.addBtnElType || 'primary'" plain icon="i-ep-plus" size="large" @click="openAddDialog">添加岗位</el-button>
		</el-col>
	</el-row>
	<el-table border stripe row-key="postId" :data="tableData" class="custom-table" @sort-change="handleTableSort" v-loading="isLoading">
		<el-table-column
			prop="postName"
			label="岗位名称"
			width="150"
	...
	/>
	<el-dialog
		:title="addOrModifyTag == 1 ? '添加岗位' : '修改岗位'"
		v-model="dialogFormAddOrModifyPost"
		width="500px"
	>
		<el-form
			:model="addOrModifyPostForm"
			:rules="addOrModifyPostFormRules"
			ref="addOrModifyPostFormRef"
			label-width="80px"
		>
		...
		<template #footer>
			<span class="dialog-footer">
				...
				<el-button type="primary" @click="handleAddOrModify(addOrModifyPostFormRef)">
					确 定
				</el-button>
			</span>
		</template>
...
import { listPagePost, addPost, modifyPost, deletePost } from "@/api/system/post/index";
...
const handleListPage = async () => {
	...
	const result = await listPagePost(tableParams.value);
	try {
		if (result.code == 200) {
...
/*********添加修改岗位*********/
//弹出框是否显示
const dialogFormAddOrModifyPost = ref(false);
//添加岗位表单初始值
	...
	await formEl.validate(async (valid) => {
		if (valid) {
			...
			if (addOrModifyTag.value == 1) {
				const result = await addPost(addOrModifyPostForm.value);
				if (result.code == 200) {
					ElMessage({
						message: "添加成功",
						type: "success",
				...
				const result = await modifyPost(addOrModifyPostForm.value);
				if (result.code == 200) {
					ElMessage({
						message: "修改成功",
						type: "success",
