<template>
	...
	<el-popover trigger="click" :width="360" popper-class="icon-picker-popper">
		<!-- 触发元素插槽 -->
		<template #reference>
		...
		<div class="icon-picker-container">
			...
			<div class="icon-grid">
				...
				>
					<!-- 鼠标悬停显示图标名称提示 -->
					<el-tooltip
						:content="registeredName.split('-').slice(2).join('-')"
						placement="top"
					>
						<el-icon :size="24">
							<!-- 使用自定义动态绑定组件，可以判断组件是否存在，防止报错 -->
...
// 生成带前缀的图标名称列表
const iconList = ref(Object.keys(ElementPlusIconsVue).map((key) => `i-ep-${key.toLowerCase()}`));
