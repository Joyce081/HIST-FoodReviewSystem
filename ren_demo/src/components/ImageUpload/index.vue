...
import type { UploadUserFile } from 'element-plus'; // 使用 Element Plus 内置类型
...
const fileList = ref<UploadUserFile[]>([]); // 声明为 UploadUserFile 数组
const uploadList = ref<UploadUserFile[]>([]); // 同上
...
const handleDelete = (file: any) => {
	const fileIndex = fileList.value.findIndex((f : any) => f.name === file.name);
	if (fileIndex > -1) {
