...
const routes: RouteRecordRaw[] = [
	{
		...
		meta: { requiresAuth: false, title:"登录" }, // 明确标记不需要认证
	},
	{
		path: "/",
		name: "mainLayout",
		...
		children: [
			...
			},
			{
				...
				meta: { requiresAuth: true, roles: [], menuShow: "/index",title:"首页" }, // 需要认证
			},
			{
				path: "system/dict",
				name: "dict",
				...
				children: [
					...
					},
					{
						...
						meta: { requiresAuth: true, roles: [], menuShow: "/system/dict",title:"字典类型" }, // 需要认证
					},
					{
						path: "dictData",
						name: "dictData",
						component: () => import("@/views/system/dict/DictData.vue"),
						meta: { requiresAuth: true, roles: [], menuShow: "/system/dict",title:"字典数据" }, // 需要认证
					},
				],
				meta: { requiresAuth: true, roles: [], menuShow: "/system/dict",title:"字典管理" }, // 需要认证
			},
			{
				path: "tool/gen/modify",
				name: "genModify",
				component: () => import("@/views/tool/gen/modify/GenEdit.vue"),
				meta: { requiresAuth: true, roles: [], menuShow: "/tool/gen",title:"代码生成" }, // 需要认证
			},
			{
				path: "system/user/profile",
				name: "userProfile",
				component: () => import("@/views/system/user/profile/Index.vue"),
				meta: { requiresAuth: true, roles: [], menuShow: "/system/user",title:"个人中心" }, // 需要认证
			},
		],
	},
	{
...
router.beforeEach(async (to, from, next) => {
	...
	// 从路由的meta中获取title，如果不存在则使用默认标题
	const { isDynamicTitle } = storeToRefs(themeStore);
	let title = '任后台管理系统';
	if(isDynamicTitle.value == true){
		title = (to.meta?.title as string) + ' - 任后台管理系统' || '任后台管理系统';
	}
