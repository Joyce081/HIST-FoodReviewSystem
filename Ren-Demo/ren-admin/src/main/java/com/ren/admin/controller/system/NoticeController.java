...
public class NoticeController extends BaseController {
    ...
    public TableDataInfo listNoticeByPage(@RequestParam(required = false) @Schema(description = "模糊查询") String searchLike) {
        IPage<Notice> noticeList = noticeService.listNoticeByPage(searchLike);
    ...
    @SecurityRequirements({@SecurityRequirement(name = "JWT")})
    @ApiResponse(responseCode = "200",description = "成功",content = @Content(mediaType = "application/json", schema = @Schema(implementation = AjaxResultSchemas.class)))
    public AjaxResult noticeDelete(@AuthenticationPrincipal LoginUser loginUser,@Schema(description = "公告标识",type = "long") long noticeId) {
