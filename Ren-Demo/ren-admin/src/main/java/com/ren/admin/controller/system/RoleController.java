...
public class RoleController extends BaseController {
    ...
    public TableDataInfo listRoleByPage(@RequestParam(required = false) @Schema(description = "模糊查询") String searchLike) {
        IPage<Role> roleList = roleService.listRoleByPage(searchLike,null);
    ...
    @SecurityRequirements({@SecurityRequirement(name = "JWT")})
    @ApiResponse(responseCode = "200",description = "成功",content = @Content(mediaType = "application/json", schema = @Schema(implementation = AjaxResultSchemas.class)))
    public AjaxResult roleDelete(@AuthenticationPrincipal LoginUser loginUser, @Schema(description = "角色标识",type = "long") long roleId) {
        //查询当前角色下是否有用户还在使用
        List<User> userList = userService.listUserByRoleId(roleId);
        if(userList != null && !userList.isEmpty()){
            return error("当前角色下还有用户在使用，不能删除");
        }
        roleService.modifyRoleIsDelById(roleId, AppConstants.COMMON_INT_YES,loginUser.getUsername());
        return success();
    }
