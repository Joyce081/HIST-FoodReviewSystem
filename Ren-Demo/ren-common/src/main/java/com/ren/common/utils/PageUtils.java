...
public class PageUtils
{
    ...
    public static <T> Page<T> createPage(Class<T> clazz) {
        ...
        if (StringUtils.isNotBlank(PageDomain.ORDER_BY_COLUMN.get()) && StringUtils.isNotBlank(PageDomain.ORDER_BY_WAY.get())) {
            ...
            String way = PageDomain.ORDER_BY_WAY.get().toLowerCase();
            OrderItem orderItem;

            if ("asc".equals(way)) {
                orderItem = OrderItem.asc(PageDomain.ORDER_BY_COLUMN.get());
            } else {
                orderItem = OrderItem.desc(PageDomain.ORDER_BY_COLUMN.get());
            }
