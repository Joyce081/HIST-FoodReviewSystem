package com.ren.common.factory;
...
public class NestedYamlPropertySourceFactory implements PropertySourceFactory {
	...
	private void flattenMap(String prefix, Map<?, ?> source, Map<String, Object> result) {
		source.forEach((key, value) -> {
			...
			Object unpackedValue = unpackOriginTrackedValue(value);

			if (unpackedValue instanceof Map) {
				// 递归处理嵌套Map
				flattenMap(fullKey, (Map<?, ?>) unpackedValue, result);
			} else {
				// 添加三种格式的属性名（您原有功能）
				// 1. 完整路径
				result.put(fullKey, unpackedValue);

				// 2. 短名前缀
				String[] parts = fullKey.split("\\.");
				if (parts.length > 1) {
					String shortKey = String.join(".", parts[parts.length - 2], parts[parts.length - 1]);
					result.putIfAbsent(shortKey, unpackedValue);
				}

				// 3. 短名无前缀
				result.putIfAbsent(parts[parts.length - 1], unpackedValue);
			}
		});
	}

	// 安全解包 OriginTrackedValue
	private Object unpackOriginTrackedValue(Object value) {
		if (value instanceof OriginTrackedValue) {
			return ((OriginTrackedValue) value).getValue(); // 提取原始值
		}
