...
import com.ren.cloudstorage.core.domain.schemas.UploadSchemas;
...
public class CloudStorageController{
    ...
    @ApiResponse(responseCode = "200",description = "成功",content = @Content(mediaType = "application/json", schema = @Schema(implementation = UploadSchemas.class)))
    public AjaxResult uploadFile(
            @Schema(description = "文件")
            MultipartFile file,
            @Schema(description = "文件需要上传至哪个目录下")
            String belong) throws OSSException {
        if (file.isEmpty()) {
    ...
    @SecurityRequirements({@SecurityRequirement(name = "JWT")})
    @ApiResponse(responseCode = "200",description = "成功",content = @Content(mediaType = "application/json", examples = @ExampleObject(value = "\"https://xueyaxuetang.oss-cn-region.aliyuncs.com/xyxt_img/article/2025/07/19/12bf8f626f354aa2aa094dca8e6f8a02.jpeg\"")))
