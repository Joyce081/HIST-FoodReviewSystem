...
@OpenAPIDefinition(
        // 多环境配置
        servers = {
                @Server(url = "${url.front}/dev-api", description = "请求前缀")
        }
)
@SecurityScheme(
        name = "JWT",                      // 认证方案名称（与接口注解一致）
        type = SecuritySchemeType.HTTP,    // 认证类型为HTTP
        scheme = "bearer",                 // 认证方案为Bearer Token
        bearerFormat = "JWT",              // Token格式为JWT
        in = SecuritySchemeIn.HEADER,      // Token传递位置在请求头
        paramName = "X-Access-Token",       // ⭐️关键：自定义请求头名称
        // 关键：添加提示文字
        description = "请进入认证模块获取测试Token后，进行认证，之后请求接口即可（注意：测试token的有效期为3小时）"
)
