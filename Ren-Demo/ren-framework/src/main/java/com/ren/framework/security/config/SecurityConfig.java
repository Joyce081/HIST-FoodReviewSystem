...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.headers(headers -> headers
            ...
            // 指定允许的嵌入源（前端端口）
            .contentSecurityPolicy(csp -> csp.policyDirectives(StringUtils.format("frame-ancestors 'self' {};", frontUrl))));
        ...
        http.authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers("/auth/login", "/auth/refreshToken", "/auth/getTestAccessToken").permitAll()
                // AI对话接口完全放开（避免SpringSecurity方法级安全冲突，关于用户身份认证会在方法内进行）
                .requestMatchers("/main/ai/mainChat").permitAll()
                // 静态资源，可匿名访问
