...
public class JwtUtils {
    ...
    public String createAccessToken(LoginUser loginUser,Long expireTime) {
        expireTime = expireTime == null ? (DateUtils.current() + tokenProperties.getAccessTokenExpireTime() * 1000L) : expireTime;
    ...
    public String createRefreshToken(LoginUser loginUser,Long expireTime, HttpServletRequest httpRequest) {
        expireTime = expireTime == null ? (DateUtils.current() + tokenProperties.getRefreshTokenExpireTime() * 1000L) : expireTime;
        String login_user = FastJSON2Utils.toJson(loginUser);
        //过滤敏感字段
        //String login_user = FastJSON2Utils.filterSensitiveFields(FastJSON2Utils.EXCLUDE_PROPERTIES,loginUser);
        ...
        // 存储到Redis，有效期比Token长一些，key为refresh_token:+用户id，value为refreshToken，有效期为配置的过期时长+60秒（为了防止早删除），时间单位为秒
        redisOperateManager.setCacheObject(redisOperateManager.getRefreshTokenKey(loginUser.getUserId(),httpRequest),refreshToken,tokenProperties.getRefreshTokenExpireTime() + 60,TimeUnit.SECONDS);
        return refreshToken;
    }
    ...
    public boolean shouldRefreshToken(String token) {
        ...
        return remainTime < tokenProperties.getAccessTokenRefreshTime() * 1000L;
    }
    ...
    public Authentication getAuthenticationByAccessToken(String token) {
        LoginUser loginUser = getLoginUserByToken(1,token);
    ...
    public Authentication getAuthenticationByRefreshToken(String token) {
        LoginUser loginUser = getLoginUserByToken(2,token);
    ...
    /**
     * 根据token 获取 LoginUser
     * @param type 1：AccessToken 2：RefreshToken
     * @param token
     * @return com.ren.common.domain.model.bo.LoginUser
     * @author ren
     * @date 2025/06/05 13:26
     */
    public LoginUser getLoginUserByToken(int type,String token) {
     ...
     */
    public String saveNewAuthenticationAndReturnAccessToken(int tokenType,String token) {
     ...
     * @return java.lang.String
     * @author ren
     * @date 2025/09/20 17:33
     */
    public Map<String,String> saveNewAuthenticationAndReturnAccessToken(LoginUser loginUser, HttpServletRequest httpRequest) {
        ...
        // 生成新的双Token
        String newAccessToken = createAccessToken(loginUser,accessTokenExpireTime);
        String newRefreshToken = createRefreshToken(loginUser,accessTokenExpireTime,httpRequest);

        //重新创建一个新的Authentication 对象（保留原始凭证和权限），存入SpringSecurity
        Authentication newAuth = new UsernamePasswordAuthenticationToken(
                loginUser,               // 新的 Principal
                null, // 原始凭证（如密码，但是此框架没有使用SpringSecurity的自动验证，所以用不到凭证，存null即可）
                loginUser.getAuthorities() // 原始权限
        );
        //将新的Authentication存入SpringSecurity
        SecurityContextHolder.getContext().setAuthentication(newAuth);
