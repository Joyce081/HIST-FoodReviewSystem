...
public class LogininforServiceImpl extends ServiceImpl<LogininforMapper, Logininfor> implements LogininforService {
     ...
     */
    @Override
    public IPage<Logininfor> listLogininforByPage(String searchLike) {
        if(StringUtils.isNotBlank(searchLike)){
            searchLike = StringUtils.format("%%{}%%",searchLike);
        }
        IPage<Logininfor> logininforList = logininforMapper.listLogininforByPage(PageUtils.createPage(Logininfor.class),searchLike);
