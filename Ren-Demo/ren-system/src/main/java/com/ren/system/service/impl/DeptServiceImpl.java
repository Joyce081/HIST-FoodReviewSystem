...
public class DeptServiceImpl extends ServiceImpl<DeptMapper, Dept> implements DeptService {
    ...
    public long addDept(Dept dept, String createBy) {
        dept.setIsDel(AppConstants.COMMON_INT_NO);
     ...
     */
    @Override
    public void modifyDeptIsDelById(long deptId, int isDel, String updateBy) {
     ...
     */
    @Override
    public List<Dept> listDeptByParam(String searchLike,Integer isStop,Long parentId) {
        if(StringUtils.isNotBlank(searchLike)){
            searchLike = StringUtils.format("%%{}%%",searchLike);
        }
        List<Dept> deptList = deptMapper.listDeptByParam( searchLike, isStop, parentId);
        return deptList;
    }
}