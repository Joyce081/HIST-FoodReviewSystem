...
public class OperLogServiceImpl extends ServiceImpl<OperLogMapper, OperLog> implements OperLogService {
     ...
     */
    @Override
    public IPage<OperLog> listOperLogByPage(String searchLike) {
        if(StringUtils.isNotBlank(searchLike)){
            searchLike = StringUtils.format("%%{}%%",searchLike);
        }
        IPage<OperLog> operLogList = operLogMapper.listOperLogByPage(PageUtils.createPage(OperLog.class),searchLike);
        return operLogList;
    }
